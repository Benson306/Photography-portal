...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["username"]))){
        ...
        $sql = "SELECT id FROM client_users WHERE username = ?";
    ...
    if(empty(trim($_POST["email"]))){
        ...
        $sql = "SELECT * FROM client_users WHERE email = '$email' ";
    ...
    if(empty(trim($_POST["password"]))){
        ...
        $password_err = "<div class='bg-danger'>Password must have atleast 6 characters.</div>";
    } else{
    ...
    if(empty($username_err) && empty($password_err) && empty($confirm_password_err) && empty($email_err)){
        ...
        $sql = "INSERT INTO client_users (email, username, password) VALUES (?, ?, ?)";
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ?>
                <script>
                window.alert("You have succesfully created A Clients' Account");
...
<body>
    <div class="wrapper">
        ...
        <form class="signup_form" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            ...
            <p>Already have an account? <a href="client_login.php"><b><u>Login here</u></b></a>.</p>
        </form>
    </div>
</body>
</html>
